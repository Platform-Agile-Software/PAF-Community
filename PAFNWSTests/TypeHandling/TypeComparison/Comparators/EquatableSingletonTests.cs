...
{
	/// <summary>
	/// This tests checks the ability to invoke the creation of singleton
	/// comparators with a closed generic.
	/// </summary>
	[TestFixture]
	public class EquatableSingletonTests
	{
		...
		#region Constructors
		static EquatableSingletonTests()
		{
		...
		{
			var isCloseTo = PAFEquatable<double>.Instance.AreEqual(s_DoubleNumberToCompare, s_DoubleNumberCloseTo);
			Assert.IsTrue(isCloseTo, "double close to");
			isCloseTo = PAFEquatable<double>.Instance.AreEqual(s_DoubleNumberToCompare, s_DoubleNumberNotCloseTo);
			Assert.IsTrue(!isCloseTo, "double not close to");
		}
		[Test]
		public void TestComplexDoubleCompare()
		{
			var isCloseTo = PAFEquatable<PAFComplexDoubleNumber>.Instance.AreEqual(s_ComplexDoubleNumberToCompare, s_ComplexDoubleNumberCloseTo);
			Assert.IsTrue(isCloseTo, "complex double close to");
			isCloseTo = PAFEquatable<PAFComplexDoubleNumber>.Instance.AreEqual(s_ComplexDoubleNumberToCompare, s_ComplexDoubleNumberNotCloseTo);
			Assert.IsTrue(!isCloseTo, "complex double not close to");
		}
		[Test]
		public void TestFloatCompare()
		{
			var isCloseTo = PAFEquatable<float>.Instance.AreEqual(s_FloatNumberToCompare, s_FloatNumberCloseTo);
			Assert.IsTrue(isCloseTo, "float close to");
			isCloseTo = PAFEquatable<float>.Instance.AreEqual(s_FloatNumberToCompare, s_FloatNumberNotCloseTo);
			Assert.IsTrue(!isCloseTo, "float not close to");
		}
		[Test]
		public void TestComplexFloatCompare()
		{
			var isCloseTo = PAFEquatable<PAFComplexFloatNumber>.Instance.AreEqual(s_ComplexFloatNumberToCompare, s_ComplexFloatNumberCloseTo);
			Assert.IsTrue(isCloseTo, "complex float close to");
			isCloseTo = PAFEquatable<PAFComplexFloatNumber>.Instance.AreEqual(s_ComplexFloatNumberToCompare,
				s_ComplexFloatNumberNotCloseTo);
			Assert.IsTrue(!isCloseTo, "complex float not close to");
		}
		/// <summary>
		/// This test ensures that we drop though to the Microsoft <see cref="EqualityComparer{T}"/>
		/// as we have designed the singleton to do when we don't have a closure for a
		/// specific type.
		/// </summary>
		[Test]
		public void TestCompareInts()
		{
			...
			var isCloseTo = PAFEquatable<DateTime>.Instance.AreEqual(now, now);
			Assert.IsTrue(isCloseTo, "date close to");
			isCloseTo = PAFEquatable<DateTime>.Instance.AreEqual(now,now + TimeSpan.FromMilliseconds(1));
			Assert.IsTrue(!isCloseTo, "date not close to");
		}
